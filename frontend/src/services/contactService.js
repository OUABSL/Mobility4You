...
contactAPI.interceptors.response.use(
  ...
  (error) => {
    ...
    if (error.code === 'ECONNABORTED') {
      error.userMessage = 'La solicitud tardó demasiado tiempo. Por favor, inténtalo de nuevo.';
    } else if (!error.response) {
      error.userMessage = 'No se puede conectar con el servidor. Verifica tu conexión a internet.';
    } else {
      switch (error.response.status) {
        case 400:
          error.userMessage = 'Datos inválidos. Por favor, revisa la información ingresada.';
          break;
        case 429:
          error.userMessage = 'Demasiadas solicitudes. Por favor, espera un momento antes de intentar de nuevo.';
          break;
        case 500:
          error.userMessage = 'Error interno del servidor. Inténtalo de nuevo más tarde.';
          break;
        default:
          error.userMessage = 'Ha ocurrido un error inesperado. Inténtalo de nuevo.';
      }
    }
...
class ContactService {
  ...
  async sendContactForm(formData) {
    try {
      ...
      // Realizar petición
      const response = await contactAPI.post('/contact/', dataToSend);
