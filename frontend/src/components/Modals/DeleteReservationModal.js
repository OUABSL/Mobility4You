...
const DeleteReservationModal = ({ show, onHide, reservationId, onConfirm }) => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setError('Error al cancelar la reserva: ' + (err.message || 'Intente nuevamente'));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Modal show={show} onHide={onHide} centered>
      ...
      <Modal.Body>
        ...
        <p>Esta acción no se puede deshacer y puede estar sujeta a penalizaciones según la política de cancelación.</p>
      </Modal.Body>
      ...
      <Modal.Footer>
        ...
        >
          {loading ? (
            ...
            <>
              <FontAwesomeIcon icon={faCheckCircle} className="me-1" /> Confirmar Cancelación
            </>
          )}
        </Button>
      </Modal.Footer>
