...
from .models import PagoStripe, ReembolsoStripe
from .serializers import PagoStripeSerializer, ReembolsoStripeSerializer
    ...
    def post(self, request):
        ...
        try:
            ...
            if resultado['success']:
                logger.info(f"Payment Intent creado exitosamente: {resultado['payment_intent_id']}")
...
class ConfirmPaymentIntentView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if resultado['success']:
                logger.info(f"Payment Intent confirmado exitosamente: {payment_intent_id}")
    ...
    def post(self, request, pago_id):
        ...
        try:
            ...
            if motivo not in motivos_validos:
                return Response(
                    {'error': f'Motivo debe ser uno de: {", ".join(motivos_validos)}'},
                    status=status.HTTP_400_BAD_REQUEST
                )
            ...
            if resultado['success']:
                logger.info(f"Reembolso procesado exitosamente: {resultado['refund_id']}")
                return Response(resultado, status=status.HTTP_201_CREATED)
            else:
...
class PaymentHistoryView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            # Serializar
            serializer = PagoStripeSerializer(pagos_pagina, many=True, context={'request': request})
    ...
    def post(self, request):
        ...
        try:
            ...
            if resultado['success']:
                ...
                return HttpResponse(
                    f"Webhook processing failed: {resultado['error']}",
                    status=400
                )
            ...
            try:
                ...
                reserva_data = {
                    ...
                    'conductor': {
                        ...
                        'nombre': payment_data.get('titular', '').split()[0] if payment_data.get('titular') else '',
                        'apellidos': ' '.join(payment_data.get('titular', '').split()[1:]) if payment_data.get('titular') else ''
                    }
                }
    ...
    try:
        ...
        if resultado['success']:
            ...
            return Response({
                'success': True,
                'order': order_number,
                'status': 'COMPLETADO' if resultado['estado'] == 'COMPLETADO' else 'PENDIENTE',
