...
class StripePaymentService:
    ...
    def crear_payment_intent(self, reserva_data, tipo_pago='INICIAL', metadata_extra=None):
        """
    ...
    def confirmar_payment_intent(self, payment_intent_id, payment_method_id=None):
        ...
        try:
            ...
            pago_stripe = PagoStripe.objects.get(stripe_payment_intent_id=payment_intent_id)
    ...
    def procesar_reembolso(self, pago_id, importe_reembolso=None, motivo='CANCELACION_CLIENTE', descripcion=None):
        """
        ...
        try:
            ...
            refund_params = {
                ...
                'amount': importe_centavos,
                'reason': settings.STRIPE_REFUND_CONFIG['reason_mapping'].get(motivo, 'requested_by_customer'),
                'metadata': {
                    'pago_id': str(pago_id),
                    'motivo_interno': motivo,
                    'reserva_id': str(pago_stripe.reserva.id) if pago_stripe.reserva else '',
                }
            }
            
            if descripcion:
                refund_params['metadata']['descripcion'] = descripcion[:500]  # Límite de Stripe
    ...
    def obtener_estado_pago(self, numero_pedido):
        ...
        try:
            ...
            return {
                ...
                'fecha_confirmacion': pago_stripe.fecha_confirmacion.isoformat() if pago_stripe.fecha_confirmacion else None,
    ...
    def _extraer_nombre_cliente(self, reserva_data):
        ...
        conductor = reserva_data.get('conductor', {}) or reserva_data.get('conductorPrincipal', {})
    ...
    def _sincronizar_pago_con_stripe(self, pago_stripe):
        ...
        try:
            payment_intent = stripe.PaymentIntent.retrieve(pago_stripe.stripe_payment_intent_id)
            pago_stripe.actualizar_desde_stripe(payment_intent)
            return True
        except Exception as e:
...
class StripeWebhookService:
    ...
    def _procesar_payment_intent_succeeded(self, event):
        ...
        try:
            ...
            logger.info(f"Payment Intent exitoso procesado via webhook: {payment_intent_id}")
            return {'success': True, 'message': 'Payment Intent exitoso procesado'}
            
        except PagoStripe.DoesNotExist:
            logger.warning(f"Payment Intent no encontrado localmente: {payment_intent_id}")
    ...
    def _procesar_payment_intent_failed(self, event):
        ...
        try:
            ...
            pago_stripe.mensaje_error = payment_intent.get('last_payment_error', {}).get('message', 'Pago fallido')
            pago_stripe.codigo_error_stripe = payment_intent.get('last_payment_error', {}).get('code')
            pago_stripe.save()
            
            logger.info(f"Payment Intent fallido procesado via webhook: {payment_intent_id}")
    ...
    def _procesar_payment_intent_canceled(self, event):
        ...
        try:
            ...
            logger.info(f"Payment Intent cancelado procesado via webhook: {payment_intent_id}")
    ...
    def _procesar_refund_created(self, event):
        ...
        try:
            ...
            reembolso_existente = ReembolsoStripe.objects.filter(stripe_refund_id=refund_id).first()
