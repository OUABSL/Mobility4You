...
class PagoStripeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = PagoStripe
        fields = [
            'id', 'numero_pedido', 'stripe_payment_intent_id', 'stripe_charge_id',
            'importe', 'moneda', 'estado', 'tipo_pago', 'stripe_status',
            'metodo_pago', 'ultimos_4_digitos', 'marca_tarjeta',
            'importe_reembolsado', 'email_cliente', 'nombre_cliente',
            'mensaje_error', 'codigo_error_stripe',
            'fecha_creacion', 'fecha_actualizacion', 'fecha_confirmacion',
            'reserva', 'es_exitoso', 'puede_reembolsar', 'importe_disponible_reembolso',
            'reserva_info', 'reembolsos_info'
        ]
        read_only_fields = [
            'id', 'stripe_payment_intent_id', 'stripe_charge_id', 'stripe_status',
            'fecha_creacion', 'fecha_actualizacion', 'fecha_confirmacion',
            'es_exitoso', 'puede_reembolsar', 'importe_disponible_reembolso'
        ]
    ...
    def get_reserva_info(self, obj):
        ...
        if obj.reserva:
            return {
                ...
                } if obj.reserva.vehiculo else None
            }
        return None
...
class PagoStripeListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = PagoStripe
        fields = [
            'id', 'numero_pedido', 'importe', 'moneda', 'estado', 'tipo_pago',
            'metodo_pago', 'ultimos_4_digitos', 'marca_tarjeta',
            'fecha_creacion', 'fecha_confirmacion', 'es_exitoso', 'vehiculo_info'
        ]
...
class ReembolsoStripeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'stripe_refund_id', 'importe', 'motivo', 'estado',
            'descripcion', 'stripe_status', 'stripe_failure_reason',
            'fecha_creacion', 'fecha_procesamiento', 'pago_info'
        ]
        read_only_fields = [
            'id', 'stripe_refund_id', 'stripe_status', 'stripe_failure_reason',
...
class WebhookStripeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = WebhookStripe
        fields = [
            'id', 'stripe_event_id', 'tipo_evento', 'procesado',
            'fecha_recepcion', 'fecha_procesamiento', 'intentos_procesamiento',
            'mensaje_error'
        ]
        read_only_fields = [
            'id', 'fecha_recepcion', 'fecha_procesamiento'
        ]
...
class CreatePaymentIntentRequestSerializer(serializers.Serializer):
    ...
    reserva_data = serializers.DictField(
        help_text="Datos completos de la reserva"
    )
    ...
    metadata_extra = serializers.DictField(
        required=False,
        help_text="Metadatos adicionales para el pago"
    )
    ...
    def validate_reserva_data(self, value):
        ...
        if not conductor or not conductor.get('email'):
            raise serializers.ValidationError(
                "Se requiere email del conductor/cliente"
            )
...
class ConfirmPaymentIntentRequestSerializer(serializers.Serializer):
    ...
    payment_method_id = serializers.CharField(
        max_length=255,
        required=False,
        help_text="ID del método de pago (opcional)"
    )
...
class RefundRequestSerializer(serializers.Serializer):
    ...
    motivo = serializers.ChoiceField(
        choices=[
            'CANCELACION_CLIENTE', 'CANCELACION_EMPRESA', 'MODIFICACION_RESERVA',
            'ERROR_PAGO', 'FRAUDE', 'OTRO'
        ],
    ...
    descripcion = serializers.CharField(
        max_length=500,
        required=False,
        help_text="Descripción adicional del reembolso"
    )
